...
public class PercursoDAO extends DAO {
    ...
    String sql_query =
        insertFactory(tabela,
            new String[] {"" + novo_percurso.getModal().getId(),
                "" + novo_percurso.getPartida().getId(), "" + novo_percurso.getDestino().getId(),
                Constants.DATETIME_FORMAT.format(novo_percurso.getHoraPartida()),
                "" + novo_percurso.getHorasDuracaoPercurso(), novo_percurso.getCodigoAeroporto(),
                codigo});
    ...
    try {
      ...
      while (result_set.next()) {
        percursos_encontrados.add(new Percurso(result_set.getInt("id_percurso"), result_set
            ...
            .getInt("horas_duracao"), result_set.getString("codigo_aeroporto"), result_set
            .getInt("id_cidade_partida"), result_set.getInt("id_cidade_chegada")));
      }
    } catch (SQLException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
  ...
  public Percurso getPercursoByCodigo(String codigo) {
    ...
    try {
      ...
      if (result_set.first()) {
        percurso_encontrado =
            new Percurso(result_set.getInt("id_percurso"), result_set.getInt("id_modal"),
                result_set.getDate("hora_partida"), result_set.getInt("horas_duracao"),
                result_set.getString("codigo_aeroporto"), result_set.getInt("id_cidade_partida"),
