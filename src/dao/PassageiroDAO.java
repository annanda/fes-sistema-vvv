...
public class PassageiroDAO extends DAO {
  ...
  private String tabela_joined_pessoas =
      "passageiros AS pas INNER JOIN pessoas AS pes ON pas.id_pessoa = pes.id_pessoa";
  ...
  public int cadastrarPassageiro(Passageiro novo_passageiro) {
    ...
    String sql_query =
        insertFactory(
            tabela,
            new String[] {Integer.toString(novo_passageiro.getParentId()),
                responsavel == null ? null : Integer.toString(responsavel.getId()),
                novo_passageiro.getCpf(), novo_passageiro.getTelefone(),
    ...
    try {
      ...
      sql_query =
          selectFactory(tabela, new String[] {"id_passageiro"}, "cpf = " + Constants.SINGLE_QUOTE
              + cpf + Constants.SINGLE_QUOTE);
  ...
  public ArrayList<Passageiro> listarPassageiros(HashMap<String, String> conditions) {
    ...
    String sql_query =
        selectFactory(tabela_joined_pessoas, new String[] {"pas.*", "pes.*"},
    ...
    try {
      ...
      while (result_set.next()) {
        ...
        passageiros_encontrados.add(new Passageiro(result_set.getInt("id_passageiro"), result_set
            .getInt("id_pessoa"), result_set.getString("nome"), result_set.getString("endereco"),
            result_set.getString("codigo"), result_set.getString("cpf"), result_set
                .getString("telefone"), result_set.getString("profissao"), result_set
                .getDate("data_de_nascimento"), responsavel));
      }
    } catch (SQLException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
  ...
  public Passageiro getPassageiroByCpf(String cpf) {
    ...
    String sql_query =
        selectFactory(tabela_joined_pessoas, new String[] {"pas.*", "pes.*"}, "cpf = "
            + Constants.SINGLE_QUOTE + cpf + Constants.SINGLE_QUOTE);
    ...
    try {
      ...
      if (result_set.first()) {
        ...
        passageiro_encontrado =
            new Passageiro(result_set.getInt("id_passageiro"), result_set.getInt("id_pessoa"),
                result_set.getString("nome"), result_set.getString("endereco"),
                result_set.getString("codigo"), result_set.getString("cpf"),
                result_set.getString("telefone"), result_set.getString("profissao"),
                result_set.getDate("data_de_nascimento"), responsavel);
      }
    } catch (SQLException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
  ...
  public Passageiro getPassageiroByCodigo(String codigo) {
    ...
    String sql_query =
        selectFactory(tabela_joined_pessoas, new String[] {"pas.*", "pes.*"}, "codigo = "
            + Constants.SINGLE_QUOTE + codigo + Constants.SINGLE_QUOTE);
    Passageiro passageiro_encontrado = null;
  ...
  public Passageiro getPassageiroById(int id) {
    ...
    String sql_query =
        selectFactory(tabela_joined_pessoas, new String[] {"pas.*", "pes.*"}, "id_passageiro = "
            + Constants.SINGLE_QUOTE + id + Constants.SINGLE_QUOTE);
    Passageiro passageiro_encontrado = null;
  ...
  public void alterarPassageiro(Passageiro passageiro_modificado) {
    ...
    String sql_query =
        updateFactory(tabela, passageiro_modificado.toHashMap(), "id_passageiro = "
            + passageiro_modificado.getId());
    connect();
    try {
      statement.executeUpdate(sql_query);
    } catch (SQLException e) {
