...
public class CidadeDAO extends DAO {
  ...
  public int cadastrarCidade(Cidade nova_cidade) {
    ...
    String sql_query =
        insertFactory(tabela, new String[] {nova_cidade.getNome(), nova_cidade.getIdentificador(),
            codigo});
      ...
      if (result_set.first()) {
        cidade_encontrada =
            new Cidade(result_set.getInt("id_cidade"), result_set.getString("nome"),
                result_set.getString("identificador"), result_set.getString("codigo"));
      }
    } catch (SQLException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
  ...
  public void alterarCidade(Cidade cidade_modificada) {
    String sql_query =
        updateFactory(tabela, cidade_modificada.toHashMap(),
            "id_cidade = " + cidade_modificada.getId());
