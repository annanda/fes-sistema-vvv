...
public class UsuarioDAO extends DAO {
  ...
  public int cadastrarUsuario(Usuario novo_usuario) {
    ...
    String sql_query =
        insertFactory(tabela,
            new String[] {"" + novo_usuario.getParentId(), email, novo_usuario.getSenha(),
    ...
    try {
      ...
      while (result_set.next()) {
        usuarios_encontrados.add(new Usuario(result_set.getInt("id_usuario"), result_set
            ...
            result_set.getString("codigo"), result_set.getString("email"), result_set
                .getString("senha"), result_set.getInt("nivel_permissao")));
      }
    } catch (SQLException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
  ...
  public Usuario getUsuarioByEmail(String email) {
    ...
    try {
      ...
      if (result_set.first()) {
        usuario_encontrado =
            new Usuario(result_set.getInt("id_usuario"), result_set.getInt("id_pessoa"),
                result_set.getString("nome"), result_set.getString("endereco"),
                result_set.getString("codigo"), result_set.getString("email"),
                result_set.getString("senha"), result_set.getInt("nivel_permissao"));
      }
    } catch (SQLException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
  ...
  public Usuario getUsuarioByCodigo(String codigo) {
    ...
    String sql_query =
        selectFactory(
            ...
            new String[] {"usu.*", "pes.*"}, "codigo = " + Constants.SINGLE_QUOTE + codigo
                + Constants.SINGLE_QUOTE);
    Usuario usuario_encontrado = null;
  ...
  public Usuario getUsuarioById(int id) {
    ...
    String sql_query =
        selectFactory(
            tabela + " AS usu INNER JOIN pessoas AS pes ON usu.id_pessoa = pes.id_pessoa",
            new String[] {"usu.*", "pes.*"}, "id_usuario = " + Constants.SINGLE_QUOTE + id
                + Constants.SINGLE_QUOTE);
    Usuario usuario_encontrado = null;
  ...
  public void alterarUsuario(Usuario usuario_modificado) {
    ...
    String sql_query =
        updateFactory(tabela, usuario_modificado.toHashMap(),
